...
export default function DeskingModule() {
  return (
    <Suspense fallback={<div className="p-20 text-center text-slate-500 text-sm">Loading deal workbench...</div>}>
      <DeskingContent />
    </Suspense>
  );
}
          ...
          <div className="grid lg:grid-cols-2 gap-3">
            ...
            <Card className="border-slate-200 dark:border-slate-700/50 rounded-xl overflow-hidden">
              ...
              {selectedCustomer ? (
                <div className="p-2.5 space-y-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p className="text-[10px] text-slate-500 mt-0.5">{selectedCustomer.creditScore}</p>
                    </div>
                  </div>
                ...
                <div className="p-5 text-center"><UserPlus className="w-5 h-5 text-slate-300 mx-auto mb-1" /><p className="text-xs text-slate-500">Select customer</p></div>
              )}
            </Card>
            ...
            <Card className="border-slate-200 dark:border-slate-700/50 rounded-xl overflow-hidden">
                ...
                {isInventoryOpen && (
                  <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden border-b border-slate-200 dark:border-slate-700/50">
                    <div className="p-2.5 space-y-2">
                      ...
                      <p className="text-[10px] text-slate-500">{filteredInventory.length} in stock</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
              {selectedVehicle ? (
                ...
                <div className="p-5 text-center"><Car className="w-5 h-5 text-slate-300 mx-auto mb-1" /><p className="text-xs text-slate-500">Select vehicle</p></div>
              )}
            </Card>
          </div>
          ...
          <Card className="border-slate-200 dark:border-slate-700/50 rounded-xl overflow-hidden">
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                      ...
                      return (
                        <div key={r.id} onClick={() => setActiveRebateIds(prev => active ? prev.filter(x => x !== r.id) : [...prev, r.id])} className={cn("p-2 rounded-lg border cursor-pointer transition-all text-xs", active ? "border-emerald-300 bg-emerald-50" : "border-slate-200 dark:border-slate-700/50 hover:border-slate-600")}>
                          ...
                          <div className="flex items-center justify-between mt-1">
                            <span className="text-[10px] text-slate-500">{r.type}{!r.stackable ? ' • Non-stack' : ''}</span>
              ...
              <TabsContent value="trade" className="m-0 p-4 space-y-4">
                <p className="text-[11px] font-semibold text-slate-500">Trade-In Vehicle</p>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex items-end pb-2">
                    <label className="text-[11px] text-slate-500 flex items-center gap-1.5 cursor-pointer">
                      <input type="checkbox" checked={taxOnTrade} onChange={(e) => setTaxOnTrade(e.target.checked)} className="rounded border-slate-600" />Tax credit on trade
                    </label>
                  </div>
                </div>
              ...
              <TabsContent value="products" className="m-0 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-[11px] font-semibold text-slate-500">{activeProductIds.length} products • Retail: ${fiProductTotal.toLocaleString()} • GP: ${fiProfit.toLocaleString()}</p>
              ...
              <TabsContent value="log" className="m-0 p-4 space-y-2">
                <p className="text-[11px] font-semibold text-slate-500 flex items-center gap-1.5"><History className="w-3 h-3" />Activity ({dealLog.length})</p>
...
function NumInput({ label, prefix, suffix, value, onChange, step, highlight }: { label: string; prefix?: string; suffix?: string; value: number; onChange: (v: number) => void; step?: number; highlight?: boolean; }) {
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        {prefix && <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-slate-500">{prefix}</span>}
        ...
        {suffix && <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-slate-500">{suffix}</span>}
      </div>
    </div>
  );
}
