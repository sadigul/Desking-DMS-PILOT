    ...
    <div className="space-y-4 max-w-full overflow-x-hidden pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Select value={filterStatus} onValueChange={setFilterStatus}>
            <SelectTrigger className="h-8 w-[140px] text-xs rounded-lg border-slate-200">
      ...
      <div className="space-y-3">
        {displayColumns.map(col => {
          ...
          return (
            <Card key={col} className="border-slate-200/60 rounded-xl overflow-hidden">
              {/* Column Header */}
              <button onClick={() => toggleCol(col)} className="w-full flex items-center justify-between p-3.5 hover:bg-slate-50/50 transition-colors">
                <div className="flex items-center gap-2.5">
                  ...
                  <Badge variant="secondary" className="text-[10px] h-5 px-1.5 bg-slate-100">{deals.length} deals</Badge>
                  <span className="text-[11px] text-slate-400 hidden sm:inline">${deals.reduce((s, d) => s + d.amount, 0).toLocaleString()} total</span>
                </div>
                ...
                <div className="px-3.5 pb-3.5 grid sm:grid-cols-2 xl:grid-cols-3 gap-2.5">
                  {deals.map(deal => (
                    <motion.div
                      ...
                      className="p-3 rounded-lg border border-slate-200/60 bg-white hover:border-slate-300 hover:shadow-sm transition-all cursor-pointer"
                    >
                      <div className="flex justify-between items-start mb-1.5">
                        <div className="min-w-0">
                          <p className="font-medium text-slate-800 text-xs truncate">{deal.customer}</p>
                          <p className="text-[10px] text-slate-400 truncate mt-0.5">{deal.vehicle}</p>
                        </div>
                      ...
                      <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-50">
                        <div className="flex items-center gap-1.5">
                          <div className="w-5 h-5 rounded-md bg-slate-100 flex items-center justify-center text-[9px] font-medium text-slate-500">
                            {deal.fiManager.charAt(0)}
                          </div>
                          <span className="text-[10px] text-slate-400 truncate max-w-[60px]">{deal.fiManager}</span>
                        </div>
                        <div className="flex items-center gap-1">
              ...
              {expanded && deals.length === 0 && (
                <div className="px-3.5 pb-3.5">
                  <p className="text-xs text-slate-400 text-center py-4">No deals in this stage</p>
                </div>
              )}
            </Card>
          );
